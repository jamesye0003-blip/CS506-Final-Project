...
def save_interactive_tsne(X_2d, y):
    ...
    fig = px.scatter(
        ...
        color=labels,
        hover_name=labels,
        title="t-SNE of YAMNet Embeddings (Interactive)",
    )
    out_html = os.path.join(SAVE_DIR, "tsne_yamnet_embeddings.html")
...
def main():
    ...
    for cid in range(10):
        ...
        plt.scatter(
            ...
            s=10, alpha=0.6, color=cmap(cid), label=CLASS_NAMES[cid]
        )
    plt.legend()
    plt.title("t-SNE of YAMNet Embeddings")
    out_png = os.path.join(SAVE_DIR, "tsne_yamnet_embeddings.png")
